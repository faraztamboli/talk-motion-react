class youtube_player {
    constructor(id_video_iframe, url, on_ready_callback, on_player_state_changed_callback) {
        ...
        this.onYouTubePlayerAPIReady(on_ready_callback, on_player_state_changed_callback);
    ...
    get_youtube_video_code(url) {
        ...
        if (is_full_url >= 0) {
            let index_of_video_code = is_full_url + 'https://www.youtube.com/watch?v='.length;
            video_code = url.substring(index_of_video_code);
        }
        else if (is_embed_url >=0) {
            let index_of_video_code = is_embed_url + 'https://www.youtube.com/embed/'.length;
            let index_of_video_code_end = url.length;
            if(url.indexOf('?enablejsapi=1&html5=1&autoplay=1')>=0) {
                index_of_video_code_end = url.length - ('?enablejsapi=1&html5=1&autoplay=1'.length);
            }
            else {
    ...
    get_embedding_youtube_url(video_code) {
        return 'https://www.youtube.com/embed/' + video_code + '?enablejsapi=1&html5=1&autoplay=1';
    }
