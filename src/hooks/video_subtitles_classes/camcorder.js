...
class camcorder {
  ...
  startNewRecordingSession() {
    ...
    this.recorder.onpause = () => {
      ...
      if (me.recorder.state === 'paused') {
        ...
        // CRITICAL: Ensure shot has an ID before finalization
        // This prevents waiting for shot ID during finalization
        let current_recording = me.recording;
        let active_shot = current_recording.get_active_shot();
        if (active_shot && (active_shot.recording_shot_id === null || active_shot.recording_shot_id === undefined)) {
          console.log("⚠️ Active shot has no ID - creating shot in database immediately...");
          const original_video_start = active_shot.original_video_start;
          const recording_start = active_shot.recording_start;
          const js_start = active_shot.js_start;
          // Create shot in database immediately
          me.open_recording_shot(
            current_recording.recording_id,
            original_video_start,
            recording_start,
              ...
              if (isValid) {
                ...
                me.finalizeWebM(recorderToFinalize, chunksToFinalize, { settleMs: 1000, maxMs: 10000 })
                  .then(async ({ blob, mimeType: finalMimeType }) => {
                    ...
                    // CRITICAL: Store blob in IndexedDB immediately after finalization (before upload)
                    // This ensures data is persisted locally even if upload fails or page is closed
                    try {
                      // Get active shot to get shot ID
                      let current_recording = me.recording;
                      let active_shot = current_recording.get_active_shot();
                      if (active_shot && active_shot.recording_shot_id) {
                        // Try to import putShotBlob dynamically
                        try {
                          const { putShotBlob } = await import('../../utils/indexedDBStorage');
                          await putShotBlob(active_shot.recording_shot_id, blob);
                          console.log("✅ Blob stored in IndexedDB immediately (shot ID:", active_shot.recording_shot_id + ")");
                        } catch (importErr) {
                          console.warn("⚠️ Could not import putShotBlob, trying window.putShotBlob:", importErr);
                          // Fallback: try window.putShotBlob if available
                          if (typeof window !== 'undefined' && window.putShotBlob) {
                            await window.putShotBlob(active_shot.recording_shot_id, blob);
                            console.log("✅ Blob stored in IndexedDB via window.putShotBlob");
                          } else {
                            console.warn("⚠️ putShotBlob not available - blob not stored in IndexedDB");
                          }
                        }
                      } else {
                        console.warn("⚠️ Active shot has no ID - cannot store blob in IndexedDB yet");
                        console.warn("   Blob will be stored when shot ID becomes available");
                      }
                    } catch (storageErr) {
                      console.error("❌ Error storing blob in IndexedDB:", storageErr);
                      // Continue with upload even if IndexedDB storage fails
                    }
  ...
  resumeRecording() {
    ...
    if (this.recorder && this.recorder.state === 'inactive' && this.isPaused) {
      ...
      // Wait for pending recorders to complete AND previous segment to be saved before starting a new session
      const checkPendingRecorders = async (attempts = 0, maxAttempts = 100) => {
        // Check if previous segment blob is saved in IndexedDB
        let previousSegmentSaved = true;
        try {
          let current_recording = this.recording;
          let previous_shot = null;
          // Find the most recent shot that was being finalized
          if (current_recording && current_recording.recording_shots.length > 0) {
            // Get the shot before the active one (if any)
            const shots = current_recording.recording_shots;
            if (shots.length > 1) {
              previous_shot = shots[shots.length - 2]; // Second to last shot
            } else if (shots.length === 1) {
              previous_shot = shots[0];
            }
            
            if (previous_shot && previous_shot.recording_shot_id) {
              // Check if blob is in IndexedDB
              try {
                const { getShotBlob } = await import('../../utils/indexedDBStorage');
                const blob = await getShotBlob(previous_shot.recording_shot_id);
                previousSegmentSaved = blob !== null;
                if (!previousSegmentSaved && attempts % 10 === 0) {
                  console.log(`⏳ Waiting for previous segment to be saved to IndexedDB (shot ID: ${previous_shot.recording_shot_id})...`);
                }
              } catch (importErr) {
                // If IndexedDB check fails, assume it's saved (don't block resume)
                console.warn("⚠️ Could not check IndexedDB for previous segment:", importErr);
                previousSegmentSaved = true;
              }
            }
          }
        } catch (err) {
          console.warn("⚠️ Error checking previous segment save status:", err);
          // Don't block resume if check fails
          previousSegmentSaved = true;
        }
        
        if (this.pendingRecorders.length === 0 && previousSegmentSaved) {
          console.log("✅ No pending recorders and previous segment saved, starting new session");
          this.isPaused = false;
          ...
          console.warn("⚠️ Timeout waiting for pending recorders/previous segment, starting new session anyway");
          console.warn("   Previous segment may not be fully saved - this could cause data loss");
          this.isPaused = false;
          this.startNewRecordingSession();
          setTimeout(() => {
            // Final check: verify video is still playing before resuming
